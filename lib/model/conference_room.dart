...
class ConferenceRoom with ChangeNotifier {
  ...
  Future<void> sendMessage(String message) async {
    ...
    if (localDataTrack == null || _messages.isNotEmpty) {
      log('ConferenceRoom.sendMessage => Track is not available yet, buffering message.');
  ...
  Future<void> toggleVideoEnabled() async {
    ...
    if (localVideoTrack == null) {
      log('ConferenceRoom.toggleVideoEnabled() => Track is not available yet!');
      return;
    }
    ...
    log('ConferenceRoom.toggleVideoEnabled() => ${localVideoTrack.isEnabled}');
  ...
  Future<void> toggleMute(RemoteParticipant remoteParticipant) async {
    ...
    var index = _participants.indexWhere((ParticipantWidget participant) =>
        participant.key == remoteParticipant.sid);
    if (index < 0) {
      return;
    }
    _participants[index] =
  ...
  Future<void> toggleAudioEnabled() async {
    ...
    log('ConferenceRoom.toggleAudioEnabled() => ${localAudioTrack.isEnabled}');
    _onAudioEnabledStreamController.add(localAudioTrack.isEnabled);
    notifyListeners();
  }

  Future<void> switchCamera() async {
    log('ConferenceRoom.switchCamera()');
    final sources = await CameraSource.getSources();
  ...
  Future<void> toggleSpeaker() async {
    log('ConferenceRoom.toggleSpeaker()');
  ...
  void removeDummy() {
    log('ConferenceRoom.removeDummy()');
    var dummy =
  ...
  void _onDisconnected(RoomDisconnectedEvent event) {
    log('ConferenceRoom._onDisconnected');
    _timer.cancel();
  }

  void _onReconnecting(RoomReconnectingEvent room) {
    log('ConferenceRoom._onReconnecting');
    _timer.cancel();
  }
  ...
  void _onConnected(Room room) {
    ...
    if (localParticipant == null) {
      log('ConferenceRoom._onConnected => localParticipant is null');
      return;
    }
    ...
    for (final remoteParticipant in room.remoteParticipants) {
      var participant = _participants.firstWhereOrNull(
          (participant) => participant.key == remoteParticipant.sid);
      if (participant == null) {
        log('Adding participant that was already present in the room ${remoteParticipant.sid}, before I connected');
  ...
  void _onLocalDataTrackPublished(LocalDataTrackPublishedEvent event) {
    ...
    while (_messages.isNotEmpty) {
      ...
      log('Sending buffered message: $message');
      event.localDataTrackPublication.localDataTrack!.send(message);
    }
  }

  void _onConnectFailure(RoomConnectFailureEvent event) {
    log('ConferenceRoom._onConnectFailure: ${event.exception}');
  ...
  void _onDominantSpeakerChanged(DominantSpeakerChangedEvent event) {
    log('ConferenceRoom._onDominantSpeakerChanged: ${event.remoteParticipant?.identity}');
  ...
  void _onParticipantConnected(RoomParticipantConnectedEvent event) {
    log('ConferenceRoom._onParticipantConnected, ${event.remoteParticipant.sid}');
  ...
  void _addRemoteParticipantListeners(RemoteParticipant remoteParticipant) {
    log('ConferenceRoom._addRemoteParticipantListeners() => Adding listeners to remoteParticipant ${remoteParticipant.sid}');
    _streamSubscriptions.add(
  ...
  void _onAudioTrackDisabled(RemoteAudioTrackEvent event) {
    log('ConferenceRoom._onAudioTrackDisabled(), ${event.remoteParticipant.sid}, ${event.remoteAudioTrackPublication.trackSid}, isEnabled: ${event.remoteAudioTrackPublication.isTrackEnabled}');
    _setRemoteAudioEnabled(event);
  }

  void _onAudioTrackEnabled(RemoteAudioTrackEvent event) {
    log('ConferenceRoom._onAudioTrackEnabled(), ${event.remoteParticipant.sid}, ${event.remoteAudioTrackPublication.trackSid}, isEnabled: ${event.remoteAudioTrackPublication.isTrackEnabled}');
    _setRemoteAudioEnabled(event);
  }

  void _onAudioTrackPublished(RemoteAudioTrackEvent event) {
    log('ConferenceRoom._onAudioTrackPublished(), ${event.remoteParticipant.sid}}');
  }

  void _onAudioTrackSubscribed(RemoteAudioTrackSubscriptionEvent event) {
    log('ConferenceRoom._onAudioTrackSubscribed(), ${event.remoteParticipant.sid}, ${event.remoteAudioTrackPublication.trackSid}');
    _addOrUpdateParticipant(event);
  }

  void _onAudioTrackSubscriptionFailed(
      RemoteAudioTrackSubscriptionFailedEvent event) {
    log('ConferenceRoom._onAudioTrackSubscriptionFailed(), ${event.remoteParticipant.sid}, ${event.remoteAudioTrackPublication.trackSid}');
  ...
  void _onAudioTrackUnpublished(RemoteAudioTrackEvent event) {
    log('ConferenceRoom._onAudioTrackUnpublished(), ${event.remoteParticipant.sid}, ${event.remoteAudioTrackPublication.trackSid}');
  }

  void _onAudioTrackUnsubscribed(RemoteAudioTrackSubscriptionEvent event) {
    log('ConferenceRoom._onAudioTrackUnsubscribed(), ${event.remoteParticipant.sid}, ${event.remoteAudioTrack.sid}');
  }

  void _onDataTrackPublished(RemoteDataTrackEvent event) {
    log('ConferenceRoom._onDataTrackPublished(), ${event.remoteParticipant.sid}}');
  }

  void _onDataTrackSubscribed(RemoteDataTrackSubscriptionEvent event) {
    log('ConferenceRoom._onDataTrackSubscribed(), ${event.remoteParticipant.sid}, ${event.remoteDataTrackPublication.trackSid}');
    final dataTrack = event.remoteDataTrackPublication.remoteDataTrack;
    if (dataTrack == null) {
      log('ConferenceRoom._onDataTrackSubscribed() => dataTrack == null');
      return;
    }
  ...
  void _onDataTrackSubscriptionFailed(
    ...
    log('ConferenceRoom._onDataTrackSubscriptionFailed(), ${event.remoteParticipant.sid}, ${event.remoteDataTrackPublication.trackSid}');
    _onExceptionStreamController.add(
      PlatformException(
  ...
  void _onDataTrackUnpublished(RemoteDataTrackEvent event) {
    log('ConferenceRoom._onDataTrackUnpublished(), ${event.remoteParticipant.sid}, ${event.remoteDataTrackPublication.trackSid}');
  }

  void _onDataTrackUnsubscribed(RemoteDataTrackSubscriptionEvent event) {
    log('ConferenceRoom._onDataTrackUnsubscribed(), ${event.remoteParticipant.sid}, ${event.remoteDataTrack.sid}');
  }

  void _onNetworkQualityChanged(RemoteNetworkQualityLevelChangedEvent event) {
    log('ConferenceRoom._onNetworkQualityChanged(), ${event.remoteParticipant.sid}, ${event.networkQualityLevel}');
  }

  void _onVideoTrackDisabled(RemoteVideoTrackEvent event) {
    log('ConferenceRoom._onVideoTrackDisabled(), ${event.remoteParticipant.sid}, ${event.remoteVideoTrackPublication.trackSid}, isEnabled: ${event.remoteVideoTrackPublication.isTrackEnabled}');
    _setRemoteVideoEnabled(event);
  }

  void _onVideoTrackEnabled(RemoteVideoTrackEvent event) {
    log('ConferenceRoom._onVideoTrackEnabled(), ${event.remoteParticipant.sid}, ${event.remoteVideoTrackPublication.trackSid}, isEnabled: ${event.remoteVideoTrackPublication.isTrackEnabled}');
    _setRemoteVideoEnabled(event);
  }

  void _onVideoTrackPublished(RemoteVideoTrackEvent event) {
    log('ConferenceRoom._onVideoTrackPublished(), ${event.remoteParticipant.sid}, ${event.remoteVideoTrackPublication.trackSid}');
  }

  void _onVideoTrackSubscribed(RemoteVideoTrackSubscriptionEvent event) {
    log('ConferenceRoom._onVideoTrackSubscribed(), ${event.remoteParticipant.sid}, ${event.remoteVideoTrack.sid}');
    _addOrUpdateParticipant(event);
  }

  void _onVideoTrackSubscriptionFailed(
      RemoteVideoTrackSubscriptionFailedEvent event) {
    log('ConferenceRoom._onVideoTrackSubscriptionFailed(), ${event.remoteParticipant.sid}, ${event.remoteVideoTrackPublication.trackSid}');
    _onExceptionStreamController.add(
      PlatformException(
  ...
  void _onVideoTrackUnpublished(RemoteVideoTrackEvent event) {
    log('ConferenceRoom._onVideoTrackUnpublished(), ${event.remoteParticipant.sid}, ${event.remoteVideoTrackPublication.trackSid}');
  }

  void _onVideoTrackUnsubscribed(RemoteVideoTrackSubscriptionEvent event) {
    log('ConferenceRoom._onVideoTrackUnsubscribed(), ${event.remoteParticipant.sid}, ${event.remoteVideoTrack.sid}');
  }

  void _onMessage(RemoteDataTrackStringMessageEvent event) {
    log('onMessage => ${event.remoteDataTrack.sid}, ${event.message}');
  }

  void _onBufferMessage(RemoteDataTrackBufferMessageEvent event) {
    log('onBufferMessage => ${event.remoteDataTrack.sid}, ${String.fromCharCodes(event.message.asUint8List())}');
  }
  ...
  void _addOrUpdateParticipant(RemoteParticipantEvent event) {
    log('ConferenceRoom._addOrUpdateParticipant(), ${event.remoteParticipant.sid}');
...
// log('Participant found: ${participant.id}, updating A/V enabled values');
      ...
      if (bufferedParticipant != null) {
        ...
        log('Audio subscription came first, waiting for the video subscription...');
      ...
      if (event is RemoteVideoTrackSubscriptionEvent) {
        log('New participant, adding: ${event.remoteParticipant.sid}');
        _participants.insert(
          0,
          _buildParticipant(
